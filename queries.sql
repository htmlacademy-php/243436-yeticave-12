...
INSERT INTO 
  user
  (created_at, email, name, password, contact)
VALUES
  (NOW(), 'mikhail@mail.ru', 'Mikhail', '0000', 'telegram: @mikhail');

INSERT INTO 
  user
  (created_at, email, name, password, contact)
VALUES
  (NOW(), 'alex@mail.ru', 'Alex', '111', 'skype: alex');

-- Добавляем существующие объявления(лоты)
INSERT INTO 
  lot
  (date_start, title, description, path, cost, date_finish, rate_step, user_id, category_id)
VALUES
  (NOW(),
...
SELECT lot.id, lot.title, category.name
...
SELECT rate.cost
FROM lot JOIN rate ON lot.id = rate.lot_id
WHERE lot.id = 2
