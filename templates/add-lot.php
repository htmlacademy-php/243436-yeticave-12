<main>
  ...
  <form class="form form--add-lot container <?= $form_invalid; ?>" action="add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
    ...
    <div class="form__container-two">
      <div class="form__item <?= $value_invalid['lot-name']; ?>">
      ...
      <div class="form__item <?= $category_invalid; ?>">
        ...
        <select id="category" name="category">
          ...
          <?php foreach ($categories as $category): ?>
            <option value="<?= $category['id']; ?>" <?= $category['id'] == $select_category ? 'selected' : ''; ?> ><?= htmlspecialchars($category['name']); ?></option>
          <?php endforeach; ?>  
        </select>
        <span class="form__error">Выберите категорию</span>
      </div>
    </div>
    <div class="form__item form__item--wide <?= $value_invalid['message']; ?>">
      <label for="message">Описание <sup>*</sup></label>
      <textarea id="message" name="message" placeholder="Напишите описание лота"><?= htmlspecialchars($message) ?></textarea>
      <span class="form__error">Напишите описание лота</span>
    </div>
    <div class="form__item form__item--file <?= $file_invalid; ?>">
    ...
    <div class="form__container-three">
      ...
      <div class="form__item <?= $date_invalid; ?>">
