<main>
    ...
    echo (int) $lot['count_rate'].' '.get_noun_plural_form((int) $lot['count_rate'], 'ставка', 'ставки', 'ставок');
} ?></span>
                      <span class="lot__cost"><?= htmlspecialchars(get_sum((int) $lot['current_price'])); ?></span>
                    </div>
                    <div class="lot__timer timer <?php [$hours, $minutes] = get_dt_range($lot['date_finish']); if ((int) $hours === 0) {
    echo 'timer--finishing';
      ...
      <?php if ((int) $pages_count > 1) : ?>
        <ul class="pagination-list">
          <li class="pagination-item pagination-item-prev"><a href="/all-lots.php?category_id=<?= (int) $category_id; ?>&page=<?php if ((int) $cur_page > 1) {
          ...
          <?php endforeach; ?>
          <li class="pagination-item pagination-item-next"><a href="/all-lots.php?category_id=<?= (int) $category_id; ?>&page=<?php if ((int) $cur_page < (int) $pages_count) {
                          echo ++$cur_page;
                      } else {
